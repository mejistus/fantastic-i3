...
if [ "$(cat /sys/class/power_supply/ADP1/online)" -eq 1 ]; then
    echo "已插电"
    echo 5 | sudo tee /sys/devices/system/cpu/cpu*/power/energy_perf_bias
    for i in /sys/devices/system/cpu/cpu*/cpufreq/scaling_max_freq; 
    do 
        echo "4200000" | sudo tee $i; 
    done 
else
    echo "未插电"
    echo 14 | sudo tee /sys/devices/system/cpu/cpu*/power/energy_perf_bias
    for i in /sys/devices/system/cpu/cpu*/cpufreq/scaling_max_freq; 
    do 
        echo "2000000" | sudo tee $i; 
    done 
